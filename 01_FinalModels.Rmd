...
model_rows_fixed_hu_ordinal <- c(
  model_rows_fixed_hu[1],
  'Intercept[1]',
  'Intercept[2]',
  'Intercept[3]',
  'Intercept[4]',
  'Intercept[5]',
  model_rows_fixed_hu[2:length(model_rows_fixed_hu)]
)
...
model_rownames_fixed_hu_ordinal <- c(
  model_rownames_fixed_hu[1],
  'Intercept[1]',
  'Intercept[2]',
  'Intercept[3]',
  'Intercept[4]',
  'Intercept[5]',
  model_rownames_fixed_hu[2:length(model_rownames_fixed_hu)]
)
...
# for linear models it is 0.1 * sd(Y) according to Kruschke (2018). 
# Thus, for the lognormal distribution 
# we need to sd of log(Y). For the continuous model. 
rope_factor <- sd(log(pa_sub$data$pa_sub[pa_sub$data$pa_sub > 0]))

df_report <- bayestestR::describe_posterior(
  pa_sub, 
  test = c(
    "p_direction" 
    ,"rope"
  ...
  rope_range = c(-0.1 * rope_factor, 0.1 * rope_factor)
) 

df_report[!grepl('hu_', df_report$Parameter), ]
...
plot(
  bayestestR::rope(
    pa_sub, 
    parameter = variable, 
    range = c(-0.1 * rope_factor, 0.1 * rope_factor)),
) + theme_bw()
...
plot(
  bayestestR::rope(pa_sub, parameter = variable, range = c(-0.18, 0.18)),
) + theme_bw()
...
slopes <- estimate_slopes(model, trend = "pressure_partner_cw", by = "coupleID")
...
# rope range for conditional model
rope_factor <- sd(log(pa_obj_log$data$pa_obj))

plot(
  bayestestR::rope(pa_obj_log, range = c(-0.1 * rope_factor, 0.1 * rope_factor))
) + theme_bw()
