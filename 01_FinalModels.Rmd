...
conds_eff <- conditional_spaghetti(
  ...
  y_limits = c(0, 100),
  filter_quantiles = 0.9995
)

svg("plot.svg", height = 11, width = 12)
...
conditional_spaghetti(
  ...
  y_label = c('Probability', 'Expected given yes', 'Combined expected total'),
  x_label = 'Positive Social Control'
  ,filter_quantiles = .9995,
  font_family = 'Candara'
)

fonts <- c('Candara', 'Segoe UI', 'Calibri', 'Cambria', 'Trebuchet MS', 'Comic Sans')

for (font_i in fonts) {
  print(font_i)
  a <- conditional_spaghetti(
    pa_sub, 
    effects = c(
      'pressure_partner_cw'
    ),
