...
summary(pa_sub, priors = TRUE, robust = TRUE)
summary(pa_sub, priors = TRUE, robust = FALSE)

bayestestR::hdi(pa_sub)
bayestestR::spi(pa_sub)
bayestestR::eti(pa_sub)
...
```{r check_pa_sub_lognormal_priorsense, cache=TRUE, cache.extra=pa_sub_digest}
if (do_priorsense) {
  priorsense_vars <- c(
      'Intercept',
...
```{r check_pa_obj_log_priorsense, cache=TRUE, cache.extra=pa_obj_log_digest}
if (do_priorsense) {
  gc()
...
```{r check_mood_gauss_priorsense, cache=TRUE, cache.extra=mood_gauss_digest}
...
```{r check_reactance_ordinal_priorsense, cache=TRUE, cache.extra=reactance_ordinal_digest}
...
```{r check_is_reactance_priorsense, cache=TRUE, cache.extra=is_reactance_digest}
if (do_priorsense) {
  gc()
