...
```
In this instance, the warning about max treedepth is a false positive. We have set
treedepth to 15, and when we check with shinystan, we see that treedepth is 
consistently between 10 and 14.
...
conditional_effects(pa_sub, effects = 'persuasion_self_cw')
conditional_effects(pa_sub, effects = 'pressure_self_cw')
...
### Modelling 0/1 
The zero vs. 1 modelling part also has high pareto-k values, but reaches the 
same conslucsions as the hurdle model.
We tried further simplifying by removing the residual AR1 correlation structure,
which led to a model with good pareto-k values, still arriving at the same
conslusion as the original hurdle model:
```{r pa_sub_separate_inactive, results='hide', message=FALSE}
...
```{r check_pa_sub_separate_inactive}
...
```{r report_pa_sub_separate_inactive, results='asis'}
...
conditional_effects(pa_sub_zero_model, effects = 'persuasion_self_cw')
conditional_effects(pa_sub_zero_model, effects = 'pressure_self_cw')

```


### Modelling active Days
```{r pa_sub_separate_active, results='hide', message=FALSE}
...
```{r check_pa_sub_separate_active}
...
```{r report_pa_sub_separate_active, results='asis'}
...
conditional_effects(pa_sub_active_model, effects = 'persuasion_self_cw')
conditional_effects(pa_sub_active_model, effects = 'pressure_self_cw')
...
conditional_effects(pa_obj_log, effects = 'persuasion_self_cw')
conditional_effects(pa_obj_log, effects = 'pressure_self_cw')
...
conditional_effects(mood_gauss, effects = 'persuasion_self_cw')
conditional_effects(mood_gauss, effects = 'pressure_self_cw')
...
conditional_effects(reactance_ordinal, effects = 'persuasion_self_cw', categorical = TRUE)
conditional_effects(reactance_ordinal, effects = 'pressure_self_cw', categorical = TRUE)
...
conditional_effects(is_reactance, effects = 'persuasion_self_cw')
conditional_effects(is_reactance, effects = 'pressure_self_cw')

```
```{r}
conditional_effects(
  is_reactance,
  effects = 'pressure_self_cw',
  ask = FALSE
)
