...
```{r report_pa_sub_lognormald, fig.height=6, fig.width=9, dpi=600, results='asis'}
svg("plot.svg", width = 9, height = 6)
...
#```{r ddd, fig.height=7, fig.width=6, dpi=600, results='asis'}

svg("plot2.svg", width = 6, height = 7)
...
draws_df <- pa_sub %>% 
  ...
  mutate(
    ...
    c(hurdle_slope, nonzero_slope, combined_slope)
  )


# Calculate 99.5th percentile cutoff for each slope type
cutoffs <- draws_df %>%
  pivot_longer(cols = c(hurdle_slope, nonzero_slope, combined_slope),
               names_to = "slope_type",
               values_to = "value") %>%
  group_by(slope_type) %>%
  summarize(cutoff = quantile(value, 0.995))

# Filter the data to only include values up to the 99.5th percentile and set the order
plot_data <- draws_df %>%
  pivot_longer(cols = c(hurdle_slope, nonzero_slope, combined_slope),
               names_to = "slope_type",
               values_to = "value") %>%
  left_join(cutoffs, by = "slope_type") %>%
  filter(value <= cutoff) %>%
  mutate(slope_type = factor(slope_type, levels = c("hurdle_slope", "nonzero_slope", "combined_slope")))



# Step 1: Create a temporary density plot grouped by 'slope_type'
temporary_plot <- ggplot(plot_data, aes(x = value, group = slope_type)) +
  geom_density()

# Step 2: Build the plot to extract density data
build <- ggplot_build(temporary_plot)

# Extract density data
density_data <- build$data[[1]]

# Map 'group' to 'slope_type'
# Assuming 'group' corresponds to the order of 'slope_type' in plot_data
slope_levels <- unique(plot_data$slope_type)
density_data <- density_data %>%
  mutate(slope_type = slope_levels[group])

# Step 3: Add 'status' based on the vertical line at x = 1.0
density_data <- density_data %>%
  mutate(status = if_else(x <= 1.0, 'left', 'right'))

# Filter for one slope type (e.g., "combined_slope")
single_density_data <- density_data %>%
  filter(slope_type == "nonzero_slope")  # Change to the desired slope type

# Plot the density for only one slope type
ggplot(single_density_data, aes(x = x, y = y, fill = status)) +
  geom_area() +
  geom_vline(xintercept = 1.0, linetype = "solid", color = "black") +  # Thick vertical line at 1.0
  scale_fill_manual(values = c("left" = "pink", "right" = "lightblue")) +  # Customize colors as desired
  theme_minimal() +
  theme(legend.position = "none") +
  labs(
    x = "Possible Parameter Values",
    y = "Density",
    title = "Posterior Density Plot for Combined Slope (99.5% cutoff)"
  ) + coord_flip()
