...
Although the model fits the data well, the high number of large pareto-k values
could indicate problems with the model, such as overfitting / overly
complex model specification not supported by the data. 

We expected that the autocorrelation might not be necessary, as we already
account for dependency in the data via random effects and by including 
"day" as a covariate. 

## Hurdle Poisson Model
```{r pa_sub_poisson_noar1, results='hide', message=FALSE}
...
pa_sub_noar1 <- my_brm(
  ...
  cores = 4,
  seed = 7777,
  file = file.path("models_cache_brms", paste0("pa_sub_hu_poisson_no_ar1", suffix))
)
...
```{r check_pa_sub_poisson_noar1}

check_brms(pa_sub_noar1, log_pp_check = TRUE)
DHARMa.check_brms.all(pa_sub_noar1, integer = TRUE)

```


```{r report_pa_sub_poisson_noar1, results='asis'}
...
pa_sub_zero_model <- my_brm(
  ...
  cores = 4,
  seed = 7777,
  file = file.path("models_cache_brms", paste0("pa_sub_hu_zero_part", suffix))
)
...
pa_sub_active_model <- my_brm(
  ...
  cores = 4,
  seed = 7777,
  file = file.path("models_cache_brms", paste0("pa_sub_hu_active_part", suffix))
)
