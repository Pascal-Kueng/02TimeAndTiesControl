---
title: "APIM - Control - MAIN Analyses"
...
pa_sub <- my_brm(
  ...
  cores = 4,
  seed = 7777,
  file = file.path("models_cache_brms", paste0("pa_sub_hu_poisson_", suffix))
)
...
pa_sub_zero_model <- my_brm(
  ...
  cores = 4,
  seed = 7777,
  file = file.path("models_cache_brms", paste0("pa_sub_hu_zero_part_", suffix))
)
...
DHARMa.check_brms.all(pa_sub_zero_model, integer = TRUE)
...
pa_sub_active_model <- my_brm(
  ...
  cores = 4,
  seed = 7777,
  file = file.path("models_cache_brms", paste0("pa_sub_hu_active_part_", suffix))
)
...
DHARMa.check_brms.all(pa_sub_active_model, integer = TRUE)
...
hist(df_double$reactance, breaks = 18) 

hist(log(df_double$reactance+0.1), breaks = 10)

hist(df_double$reactance[df_double$reactance>0], breaks = 18)
...
reactance_ordinal <- my_brm(
  ...
  cores = 4,
  seed = 7777,
  file = file.path("models_cache_brms", paste0("reactance_ordinal_", suffix))
)
...
```{r reactance_ordinal_hu, results='hide', message=FALSE}
...
prior1 <- c(
  brms::set_prior("normal(0, 2.5)", class = "b")
  #, brms::set_prior("normal(0, 10)", class = "Intercept", lb=0, ub=5) # range of the outcome scale
  , brms::set_prior("normal(6, 6)", class = "Intercept", dpar = 'hu') # hurdle part
...
brms::validate_prior(
  ...
  formula = formula, 
  data = df_double, 
  family = hurdle_cumulative() # HURDLE_CUMULATIVE
  )


#df_minimal <- df_double[, c("AorB", all.vars(as.formula(formula)))]

reactance_ordinal_hu <- my_brm(
  mi = use_mi, 
  imputed_data = implist,
  ...
  cores = 4,
  seed = 7777,
  file = file.path("models_cache_brms", paste0("reactance_ordinal_hu_", suffix))
)
...
```{r check_reactance_ordinal_hu}

check_brms(reactance_ordinal_hu)

```

```{r report_reactance_ordinal_hu, results='asis'}

summarize_brms(
  reactance_ordinal_hu, 
  model_rows_fixed = model_rows_fixed_ordinal,
  model_rows_random = model_rows_random_ordinal,
  model_rownames_fixed = model_rownames_fixed_ordinal,
  model_rownames_random = model_rownames_random_ordinal,
...
```{r testingdifferencecoefsordinal_hu}

hypothesis(reactance_ordinal_hu, "pressure_self_cw > pushing_self_cw")
...
is_reactance <- my_brm(
  ...
  cores = 4,
  seed = 7777,
  file = file.path("models_cache_brms", paste0("is_reactance_", suffix))
  #, file_refit = 'always'
)
