...
if (corstr == 'ar') {
  ...
  autocor_prior <- brms::set_prior("normal(0, 5)", class = "ar", lb = -1, ub = 1) 
  suffix = paste0('ar_', as.character(iterations))
} else if (corstr == 'cosy_couple') {
  autocor_str <- ~ cosy(gr = coupleID)
  autocor_prior <- brms::set_prior("normal(0, 5)", class = "cosy", lb = -1, ub = 1)
  ...
  autocor_prior <- brms::set_prior("normal(0, 5)", class = "ma", lb = -1, ub = 1)
...
pa_sub <- my_brm(
  ...
  cores = 4,
  seed = 7777,
  file = file.path("models_cache_brms", paste0("pa_sub_hu_", suffix))
)
...
prior1 <- c(
  ...
  brms::set_prior("normal(0, 10)", class = "sd", group = "coupleID", lb = 0), # Random effects SD for couple
  autocor_prior
)
