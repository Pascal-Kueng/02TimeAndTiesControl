...
prior1 <- c(
  # — Population‐level slopes on μ (aff/5)
  set_prior("normal(0, 1)",    class = "b"),
  
  # — μ intercept
  #   Student‑t(3,0,2.5) is a bit wider than N(0,1) but still rules out extreme logits.
  set_prior("student_t(3, 0, 2.5)", class = "Intercept"),
  
  # — Dispersion φ on log scale
  #   Gamma(2,0.1) centers φ around 20 (i.e. fairly peaked betas), but has heavy tail.
  set_prior("gamma(2, 0.1)",   class = "phi"),
  
  # — Group‐level SDs (coupleID and coupleID:userID)
  #   Student‑t(3,0,2.5) half‑Student for SDs: most mass < 5 logits of variation.
  set_prior("student_t(3, 0, 2.5)", class = "sd"),
  
  # — Correlations among random effects
  #   LKJ(2) mildly favors lower correlations over LKJ(1) (flat).
  set_prior("lkj(2)",           class = "cor"),
  
    # — Inflation‐at‐bound intercepts
  #    P(y∈{0,1}) ≈ logit⁻¹(–1.4) ≃ 0.2
  set_prior("normal(-1.4, 1)",      class = "Intercept", dpar = "zoi"),

  #    P(y=1 | y∈{0,1}) ≈ logit⁻¹(1.4)  ≃ 0.8
  set_prior("normal(1.4, 1)",       class = "Intercept", dpar = "coi")
)
...
mood_gauss <- my_brm(
  ...
  cores = 4,
  seed = 7777,
  file = file.path("models_cache_brms", paste0("mood_beta", suffix))
)

mood_gauss_digest <- digest::digest(mood_beta)
...
```{r check_mood_beta}
if (check_models) {
  check_brms(mood_beta, log_pp_check = FALSE)
  DHARMa.check_brms.all(mood_beta, integer = FALSE)
}
```
...
plot(
  bayestestR::p_direction(mood_beta),
  priors = TRUE
)  + 
  coord_cartesian(xlim = c(-3, 3)) +
  theme_bw()
...
plot(
  bayestestR::rope(mood_beta, ci = 1)
) + theme_bw()
