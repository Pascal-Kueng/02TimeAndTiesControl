...
# Assuming your model is stored in 'pa_sub' and your data in 'df_double'
moderator <- "barriers_self_cw"  

# Generate 100 moderator values within its range
moderator_seq <- seq(
  from = min(df_double[[moderator]], na.rm = TRUE),
  to   = max(df_double[[moderator]], na.rm = TRUE),
  length.out = 1000
)
...
post_draws <- spread_draws(
  pa_sub,
  b_hu_pushing_self_cw,
  `b_hu_barriers_self_cw:pushing_self_cw`
)
...
slope_list <- lapply(moderator_seq, function(mod_val) {
  tibble(
    ...
    slope = -(post_draws$b_hu_pushing_self_cw + mod_val * post_draws$`b_hu_barriers_self_cw:pushing_self_cw`)
  )
})
...
# 1) Make sure slope_summary is sorted
...
# 3) Duplicate boundary rows for each category
...
# 4) Combine original + duplicated boundary rows
slope_summary_extended <- bind_rows(slope_summary, boundary_rows_next)

# 5) Split into "sig" and "ns"
...
# 6) Plot with a legend for significance
p <- ggplot() +
  # --- Non-significant region ---
  geom_ribbon(
    data = slope_summary_ns,
    aes(x = moderator_value, ymin = slope_lower, ymax = slope_upper,
        fill = "Non-significant"), 
    alpha = 0.2
  ) +
  geom_line(
    data = slope_summary_ns,
    aes(x = moderator_value, y = slope_median,
  ...
  scale_fill_manual(
    values = c("Significant" = "steelblue",
  ...
  labs(
    x = "Daily Barriers",
    y = "Slope (Plan-Related Pushing)",
    fill = "Significance",       # legend title for ribbons
    color = "Significance"       # legend title for lines
  ) +
  theme_minimal()
